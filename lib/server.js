...
let watchFiles = ({ directory, websitename, uploads }) => {
  ...
  chokidar.watch('.', Object.assign({
    ...
    .on('all', (event, file) => {
      let shouldRecompile = ['.html', '.pug', '.handlebars']
...
module.exports = (options) => {
  let { port, directory, adminurl, name, uploads, salt } = options
  let app = express()
  ...
  adminApp.use('/api', admin.api({ directory, websitename: name, uploads, salt, adminurl }))
