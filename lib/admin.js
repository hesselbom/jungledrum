...
const setNestedJsonValue = (json, path, value) => {
  let obj
  try { obj = JSON.parse(json) } catch (err) { obj = {} }
  let field = path[0]
  if (!obj.hasOwnProperty(field)) obj[field] = {}
  if (path.length === 1) {
    obj[field] = value
  } else {
    obj[field] = setNestedJsonValue(obj[field], path.slice(1), value)
  }
...
module.exports = {
  ...
  api: ({ directory, websitename, uploads, salt, adminurl }) => {
    ...
    app.post('/pages', upload.any(), (req, res) => {
      let _db = db.read(directory)
      let data = req.body
      let setAsHome = false
      let files = req.files || []
      ...
      files.forEach(file => {
        ...
        if (field.indexOf('$') > -1) {
          // Try to place value in correct path
          let path = field.split('$')
          let actualfield = path[0]
          let json = setNestedJsonValue(data[actualfield], path.slice(1), file.filename)
          data[actualfield] = json
        } else {
    ...
    app.put('/pages/:pageid', upload.any(), (req, res) => {
      let _db = db.read(directory)
      let data = req.body
